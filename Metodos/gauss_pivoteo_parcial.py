...
def gauss_pivoteo_parcial(A, b):
    ...
    for k in range(n - 1):
        # Buscar fila con mayor valor absoluto en columna k
        max_fila = max(range(k, n), key=lambda i: abs(M[i][k]))
        if max_fila != k:
            M[k], M[max_fila] = M[max_fila], M[k]

        for i in range(k + 1, n):
            if M[k][k] == 0:
                print(f"Error: pivote cero en columna {k}.")
                return None
            factor = M[i][k] / M[k][k]
...
def main():
    print("="*60)
    print(" GAUSS PIVOTEO PARCIAL - Entrada de datos")
    print("="*60)
